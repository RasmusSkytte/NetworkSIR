...
def plot_simulation_cases(total_tests, t_day, axes, color='k', linestyle='-', label='') :

    # Create the plots
    return axes.plot(t_day[:len(total_tests)], total_tests, lw=4, linestyle=linestyle, c=color, label=label)[0]
